...
const slides = [
  ...
  { eyebrow: "Actual tasks Mary can do", title: "Mary is not just a chatbot.", subtitle: "She can support real customer-service and commercial workflows when configured with the institution's approved information.", time: "1.5 min", visual: "tasks", bullets: ["Review customer details and submitted information", "Send quotations and invoices", "Send payment reminders", "Send receipts after authorised verification", "Schedule follow-ups", "Maintain conversation history"] },
  { eyebrow: "Workflow demonstration", title: "One customer journey, clearly controlled.", subtitle: "Mary moves each customer from enquiry to next action without losing context.", time: "1 min", visual: "workflow", bullets: ["Enquiry", "Qualification", "Quotation or invoice", "Payment proof review", "Verified receipt", "Human handover"] },
  { eyebrow: "Loan and microfinance use case", title: "Mary can organise loan enquiries before staff review.", subtitle: "Mary can explain approved products, collect basic details, list required documents and route qualified leads to loan officers. Formal credit decisions remain with authorised staff.", time: "1.5 min", visual: "finance", bullets: ["Pre-qualification support", "Document requirement guidance", "Repayment and product information", "Application-status support if integrated", "Human approval for credit decisions"] },
  { eyebrow: "Delivery, logistics and retail use case", title: "Mary can turn enquiries into organised orders.", subtitle: "For delivery and retail businesses, Mary can capture customer needs, explain approved prices, send quotations and follow up on incomplete orders.", time: "1 min", visual: "retail", bullets: ["Pickup and delivery details", "Product, size and colour questions", "Stock or tracking support where integrated", "Invoice and receipt workflows", "Follow-up on interested customers"] },
...
function Visual({ type, openOnboarding }: { type: string; openOnboarding: () => void }) {
  ...
  if (type === "dashboard") {
    return <div className={styles.dashboardMock}><div><strong>83</strong><span>Leads</span></div><div><strong>8</strong><span>Hot leads</span></div><div><strong>K28,460</strong><span>Quoted value</span></div><div><strong>80</strong><span>Follow-ups due</span></div></div>;
  }
  if (type === "tasks") {
    return <div className={styles.taskTiles}>{["Quote", "Invoice", "Receipt", "Reminder", "Review", "Handover"].map((item) => <span key={item}>{item}</span>)}</div>;
  }
  if (type === "scale") {
    return <div className={styles.scaleVisual}><strong>2,500</strong><span>client conversations</span><p>capacity target after workflow and infrastructure validation</p></div>;
  }
