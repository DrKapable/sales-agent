...
export async function GET(request: Request) {
  ...
  if (!config.oauthConfigured) {
    return NextResponse.redirect(new URL("/admin/social?facebook=setup-required", publicOrigin(request)));
  }
  ...
  oauth.searchParams.set("state", createFacebookOAuthState("/admin/social"));
