...
import { buildCommercialPdf, commercialDocumentNumber, isInvoiceStatus } from "@/lib/commercial-document";

export async function GET(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  const { id } = await context.params;
  if (!/^[0-9a-f-]{36}$/i.test(id)) return NextResponse.json({ error: "Document not found." }, { status: 404 });
  const snapshot = await getBusinessSnapshot();
  const record = snapshot.quotes.find((item: any) => item.id === id && ["QUOTATION", "INVOICE_UNPAID", "INVOICE_PAID"].includes(item.status));
  ...
  const invoice = isInvoiceStatus(record.status);
  const filename = `MedMinds-${invoice ? "Invoice" : "Quotation"}-${number}.pdf`;
