...
export function inferConversationAnswerPatch(
  ...
  if (!clean || CLARIFICATION_REQUEST.test(clean) || NO_ANSWER.test(clean) || lowInformation(clean)) return {};
  ...
  if (kind === "programme" && !lead.programme && clean.length <= 180 && !clean.includes("?")) {
    const looksLikeAnswer = PROGRAMME_LEVEL.test(clean) || /^[\p{L}\p{N}][\p{L}\p{N}\s&.,'’()/-]{1,90}$/u.test(clean);
    if (looksLikeAnswer) patch.programme = normalizeProgrammeAnswer(clean);
  }
