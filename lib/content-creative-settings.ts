...
export async function getCreativeSettings(postId: string): Promise<ContentCreativeSettings> {
  ...
  const rows = await db.query(`SELECT post_id,text_scale,support_lines,photo_x,photo_y,photo_zoom,card_opacity,logo_position,text_align,show_website,updated_at
...
export async function saveCreativeSettings(postId: string, input: Partial<ContentCreativeSettings>) {
  ...
  const rows = await db.query(`INSERT INTO medminds_content_creative_settings (
      post_id,text_scale,support_lines,photo_x,photo_y,photo_zoom,card_opacity,logo_position,text_align,show_website,updated_at
    ...
    ON CONFLICT (post_id) DO UPDATE SET
      ...
      text_align=EXCLUDED.text_align,show_website=EXCLUDED.show_website,updated_at=NOW()
    RETURNING post_id,text_scale,support_lines,photo_x,photo_y,photo_zoom,card_opacity,logo_position,text_align,show_website,updated_at`,
  [postId,item.textScale,item.supportLines,item.photoX,item.photoY,item.photoZoom,item.cardOpacity,item.logoPosition,item.textAlign,item.showWebsite]);
  const row = rows[0] as Record<string, unknown>;
  return normalize(postId, {
    ...
    textAlign: String(row.text_align) as CreativeTextAlign, showWebsite: Boolean(row.show_website), updatedAt: String(row.updated_at)
  });
}
