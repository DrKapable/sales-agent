...
const marketingTeam = ["Dr Kanyembo Ng'andwe", "Mr Conrad Mununkha Phiri", "Dr Zabibu Nandazi"] as const;
...
export function HumanFollowUpPanel({ onBack }: { onBack?: () => void }) {
  ...
  const [manualBy, setManualBy] = useState<(typeof marketingTeam)[number]>(marketingTeam[0]);
  ...
  async function post(body: Record<string, unknown>) {
    const response = await fetch("/api/admin/follow-ups", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    const json = await response.json();
  ...
  async function scheduleManual() {
    ...
    try {
      await post({ action: "schedule", phone: manualPhone, scheduledAt: new Date(manualAt).toISOString(), reason: manualReason.trim() || undefined, createdBy: manualBy });
      setNotice("Follow-up scheduled and added to the human queue.");
      setManualOpen(false); setManualPhone(""); setManualReason(""); setManualAt(toLocalInput());
      await load(true);
    } catch (err) { setError(err instanceof Error ? err.message : "Unable to schedule follow-up."); }
  ...
  return <main className="hfuShell">
    <div className="hfuContainer">
      <header className="hfuHeader">
        <div><button type="button" className="hfuBack" onClick={onBack}>← Business dashboard</button><div className="hfuTitle"><h1>Human follow-ups</h1><span>MARKETING WORKSPACE</span></div><p>Mary schedules the right clients. Marketing owns the contact, outcome and next follow-up.</p></div>
        <div className="hfuHeaderActions"><button type="button" className="hfuGhost" onClick={() => void load()} disabled={loading}>↻ Refresh</button><button type="button" className="hfuPrimary" onClick={() => setManualOpen((value) => !value)}>＋ Schedule follow-up</button></div>
      </header>
      ...
      {manualOpen ? <section className="hfuManual">
        <div><h2>Schedule a client manually</h2><p>Add an active client to the human queue or bring a client back into follow-up.</p></div>
        <div className="hfuManualGrid">
          ...
          <label>Date & time<input type="datetime-local" value={manualAt} onChange={(event) => setManualAt(event.target.value)} /></label>
          <label>Scheduled by<select value={manualBy} onChange={(event) => setManualBy(event.target.value as (typeof marketingTeam)[number])}>{marketingTeam.map((name) => <option key={name}>{name}</option>)}</select></label>
      ...
      <div className="hfuToolbar"><div className="hfuFilters" role="tablist" aria-label="Follow-up status"><button className={filter === "DUE" ? "active" : ""} onClick={() => setFilter("DUE")}>Due</button><button className={filter === "UPCOMING" ? "active" : ""} onClick={() => setFilter("UPCOMING")}>Upcoming</button><button className={filter === "COMPLETED" ? "active" : ""} onClick={() => setFilter("COMPLETED")}>Completed</button><button className={filter === "DROPPED" ? "active" : ""} onClick={() => setFilter("DROPPED")}>Dropped</button><button className={filter === "ALL" ? "active" : ""} onClick={() => setFilter("ALL")}>All</button></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search client, number or service" /></div>
...
function FollowUpCard({ task, smsConfigured, post, onChanged, onError }: { task: FollowUpTask; smsConfigured: boolean; post: (body: Record<string, unknown>) => Promise<any>; onChanged: (message: string) => Promise<void>; onError: (message: string) => void }) {
  const lead = task.lead;
  ...
  const [completedBy, setCompletedBy] = useState<(typeof marketingTeam)[number]>(marketingTeam[0]);
  ...
  async function sendSms() {
    ...
    } catch (err) { onError(err instanceof Error ? err.message : "Unable to send SMS."); }
    finally { setBusy(false); }
  }

  async function complete() {
    if (!summary.trim()) { onError("Add a short follow-up summary before ticking it complete."); return; }
    setBusy(true);
    try {
      await post({ action: "complete", taskId: task.id, completedBy, channel, summary, outcome, nextMode, nextAt: nextMode === "manual" ? new Date(nextAt).toISOString() : null });
      await onChanged(nextMode === "drop" ? "Client removed from the follow-up queue." : nextMode === "manual" ? "Follow-up completed and the next date was scheduled." : "Follow-up completed. The next follow-up is scheduled for tomorrow.");
    } catch (err) { onError(err instanceof Error ? err.message : "Unable to complete follow-up."); }
    finally { setBusy(false); }
  }

  return <article className={`hfuCard ${task.dueState === "OVERDUE" ? "overdue" : ""} ${task.status === "COMPLETED" ? "done" : ""} ${task.status === "DROPPED" ? "dropped" : ""}`}>
    <div className="hfuCardTop"><div className="hfuIdentity"><span className="hfuAvatar">{initials(lead?.name)}</span><div className="hfuIdentityCopy"><strong>{lead?.name || task.phone}</strong><span>{lead?.serviceInterest || lead?.packageName || "Service not established"} · {task.phone}</span><span>{lead?.status || "Lead record unavailable"}{lead?.programme ? ` · ${lead.programme}` : ""}</span></div></div><div className="hfuBadges"><span className={`hfuBadge ${task.dueState === "OVERDUE" ? "overdue" : task.dueState === "DUE_TODAY" ? "today" : "upcoming"}`}>{dueLabel(task)}</span>{lead?.priority ? <span className={`hfuBadge ${lead.priority.toLowerCase()}`}>{lead.priority}</span> : null}<span className="hfuBadge">Step {task.sequenceStep}</span></div></div>
    ...
    {!closed ? <>
      <div className="hfuActions"><a className="hfuAction" href={`tel:+${task.phone.replace(/\D/g, "")}`} onClick={() => setChannel("CALL")}>☎ Call</a><a className="hfuAction whatsapp" href={whatsappUrl(task)} target="_blank" rel="noreferrer" onClick={() => setChannel("WHATSAPP")}>◉ WhatsApp</a><button type="button" className="hfuAction sms" disabled={!smsConfigured} title={smsConfigured ? "Send SMS through Africa’s Talking" : "Configure Africa’s Talking first"} onClick={() => { setSmsOpen((value) => !value); setChannel("SMS"); }}>✉ SMS</button></div>
      ...
      <details className="hfuLog"><summary><span>✓ Log and complete this follow-up</span></summary><div className="hfuForm">
        <label>Completed by<select value={completedBy} onChange={(event) => setCompletedBy(event.target.value as (typeof marketingTeam)[number])}>{marketingTeam.map((name) => <option key={name}>{name}</option>)}</select></label>
        ...
        {nextMode === "manual" ? <label className="wide">Next follow-up<input type="datetime-local" value={nextAt} onChange={(event) => setNextAt(event.target.value)} /></label> : null}
