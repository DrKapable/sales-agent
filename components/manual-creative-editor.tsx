...
type EditorSnapshot = {
  settings: Settings; headline: string; support: string; cta: string; template: Template;
};
...
export function ManualCreativeEditor({ postId }: { postId: string }) {
  ...
  function snapshot(nextSettings=settings,nextHeadline=headline,nextSupport=support,nextCta=cta,nextTemplate=template):EditorSnapshot {
    return { settings: nextSettings, headline: nextHeadline, support: nextSupport, cta: nextCta, template: nextTemplate };
  }
  ...
  async function load(){
    ...
    try{
      ...
      const loadedCta=d.post.creativeCta||"Message MedMinds";
      const loadedTemplate=(d.post.creativeTemplate||"promo-clean") as Template;
      setPost(d.post); setSettings(loadedSettings); setHeadline(loadedHeadline); setSupport(loadedSupport); setCta(loadedCta); setTemplate(loadedTemplate);
      setSavedSnapshot({settings:loadedSettings,headline:loadedHeadline,support:loadedSupport,cta:loadedCta,template:loadedTemplate});
  ...
  const currentSnapshot=useMemo(()=>({settings,headline,support,cta,template}),[settings,headline,support,cta,template]);
  ...
  },[post,headline,support,cta,template,settings]);
  ...
  async function save(){
    ...
    try{
      ...
      const nextSnapshot={settings:nextSettings,headline, support, cta, template};
      setSavedSnapshot(nextSnapshot);
      setPreview(`${d.previewUrl}&editor=${Date.now()}`);
      setNotice("Saved. This exact creative version is now attached to the post and will be used by Facebook Scheduler when you schedule or post it.");
    }catch(e){setError(e instanceof Error?e.message:"Unable to save image edits.");}
    finally{setSaving(false);}
  }
  function reset(){
    if(!savedSnapshot)return;
    setSettings(savedSnapshot.settings);setHeadline(savedSnapshot.headline);setSupport(savedSnapshot.support);setCta(savedSnapshot.cta);setTemplate(savedSnapshot.template);setError("");setNotice("Unsaved edits reset to the last saved version.");
  }
  ...
  return <main className={styles.page}><div className={styles.shell}>
    <header className={styles.header}><div><Link href="/admin/social">← Facebook Scheduler</Link><span>Manual creative editor</span><h1>{post.title}</h1><p>Adjust the generated creative with a live preview. Nothing changes in the scheduler until you press <strong>Save final image edits</strong>.</p></div><div className={styles.headerActions}><Link href="/admin/content">Content Studio</Link><Link className={styles.canvaLink} href={`/admin/content/${postId}/canva`}>Canva templates</Link></div></header>
    ...
    <div className={styles.workspace}>
      ...
      <section className={styles.controls}>
        ...
        <div className={styles.block}><h2>Text</h2><label>Headline <span>{headline.length}/120</span><input value={headline} maxLength={120} onChange={e=>{setHeadline(e.target.value);setNotice("");}}/></label><label>Supporting text <span>{support.length}/320</span><textarea value={support} maxLength={320} onChange={e=>{setSupport(e.target.value);setNotice("");}}/></label><label>CTA<input value={cta} maxLength={60} onChange={e=>{setCta(e.target.value);setNotice("");}}/></label></div>
        ...
        {post.creativeVisualMode==="photo"&&<div className={styles.block}><h2>Photo framing</h2><p className={styles.blockNote}>These controls now update the preview while you drag them.</p><label>Horizontal focus <b>{settings.photoX}%</b><input type="range" min="0" max="100" value={settings.photoX} onChange={e=>patch("photoX",Number(e.target.value))}/></label><label>Vertical focus <b>{settings.photoY}%</b><input type="range" min="0" max="100" value={settings.photoY} onChange={e=>patch("photoY",Number(e.target.value))}/></label><label>Zoom <b>{settings.photoZoom.toFixed(2)}×</b><input type="range" min="1" max="1.45" step="0.05" value={settings.photoZoom} onChange={e=>patch("photoZoom",Number(e.target.value))}/></label><label>Text-card opacity <b>{Math.round(settings.cardOpacity*100)}%</b><input type="range" min="0.72" max="1" step="0.02" value={settings.cardOpacity} onChange={e=>patch("cardOpacity",Number(e.target.value))}/></label></div>}
        ...
        <p className={styles.help}>The preview is temporary until you save. After saving, return to Facebook Scheduler; future schedules and “Post now” will use the saved version. Existing frozen/scheduled Facebook publications are intentionally not changed automatically.</p>
      </section>
    </div>
  </div></main>;
}
