...
export function AdminDashboard({ initialLeads, initialOffers, setup }: { initialLeads: Lead[]; initialOffers: Offer[]; setup: Setup }) {
  ...
  const visibleOffers = useMemo(() => offers.filter((offer) => {
    const categoryMatches = offerCategory === "All categories" || offer.category === offerCategory;
    const text = `${offer.name} ${offer.category} ${offer.description}`.toLowerCase();
    return categoryMatches && text.includes(offerQuery.trim().toLowerCase());
  ...
  return <main className="dashboard">
    ...
    <section className="dashboardMain">
      ...
      {tab === "offers" && <>
        <div className="offerToolbar"><input value={offerQuery} onChange={(event) => setOfferQuery(event.target.value)} placeholder="Search services" aria-label="Search services" /><select value={offerCategory} onChange={(event) => setOfferCategory(event.target.value)} aria-label="Filter services by category">{offerCategories.map((category) => <option key={category}>{category}</option>)}</select><span>{visibleOffers.length} of {offers.length} services</span></div>
        <div className="offerGrid">{visibleOffers.map((offer) => <OfferEditor key={offer.slug} offer={offer} onSave={saveOffer} />)}</div>
      </>}
...
function OfferEditor({ offer, onSave }: { offer: Offer; onSave: (offer: Offer) => Promise<void> }) {
  ...
  return <article className="offerCard"><div className="offerTop"><div><span>{draft.category}</span><h3>{draft.name}</h3></div><label className="switch"><input type="checkbox" checked={draft.active} onChange={(e) => setDraft({ ...draft, active: e.target.checked })} /><span /></label></div>
    <div className="priceGrid"><label>Standard price, ZMW (14 days)<input type="number" min="0" step="0.01" value={draft.priceZmw ?? ""} onChange={(e) => setDraft({ ...draft, priceZmw: e.target.value ? Number(e.target.value) : null })} placeholder="Human quote" /></label><label>Rush price, ZMW (under 14 days)<input type="number" min="0" step="0.01" value={draft.rushPriceZmw ?? ""} onChange={(e) => setDraft({ ...draft, rushPriceZmw: e.target.value ? Number(e.target.value) : null })} placeholder="Human quote" /></label></div>
    ...
    <button className="button buttonPrimary" onClick={async () => { await onSave(draft); setSaved(true); setTimeout(() => setSaved(false), 1800); }} disabled={draft.active && !draft.paymentInstructions}>{saved ? "Saved" : "Save approved offer"}</button>
  </article>;
}
